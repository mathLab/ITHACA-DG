...
void steadyNS::projectPPE(fileName folder, label NU, label NP, label NSUP)
{
    ...
    // Added
    L_matrix = laplacian_velocity(NUmodes, NPmodes, NSUPmodes);
...
Eigen::MatrixXd steadyNS::laplacian_velocity(label NUmodes, label NPmodes, label NSUPmodes)
{

    label Lsize = NUmodes+NSUPmodes+liftfield.size();
    scalar mu_fake = 1.0;
...
//         for (label j = 0; j < Msize; j++)
//         {
//             L_matrix(i, j) = fvc::domainIntegrate(Together[i] & Together[j]).value();
//         }
//     }
    ...
    for (label i = 0; i < Lsize; i++)
    {
        ...
        {
//             L_matrix(i, j) = fvc::domainIntegrate(fvc::grad(Umodes[i])&fvc::grad(Umodes[j])).value();
//                     L_matrix(i, j) = fvc::domainIntegrate( Umodes[i] * fvc::laplacian(mu_fake, Umodes[j])).value();
         L_matrix(i, j) =  fvc::domainIntegrate(Together[i] & fvc::laplacian(Together[j])).value();
//             L_matrix(i, j) = fvc::domainIntegrate(Together[i]*fvc::laplacian(mu_fake, Together[j])).value();
        }
    }

    //Export the matrix
    ITHACAstream::exportMatrix(L_matrix, "L", "python", "./ITHACAoutput/Matrices/");
    ITHACAstream::exportMatrix(L_matrix, "L", "matlab", "./ITHACAoutput/Matrices/");
    ITHACAstream::exportMatrix(L_matrix, "L", "eigen", "./ITHACAoutput/Matrices/");
    return L_matrix;
}
